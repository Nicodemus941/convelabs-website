-- ========================================
-- PHASE 2B: AI LEAD SCORING ENGINE TABLES
-- ========================================
...
-- Insert Florida luxury zipcode data
...
BEGIN
  ...
  behavioral_points := behavioral_points + LEAST(15, profile_record.total_time_seconds / 12.0); -- 1 point per 12 seconds, max 15
  
  -- Page views (0-10 points)
  behavioral_points := behavioral_points + LEAST(10, profile_record.total_page_views * 2); -- 2 points per page, max 10
  
  -- Services viewed (0-10 points)
  behavioral_points := behavioral_points + LEAST(10, array_length(profile_record.services_viewed, 1) * 3); -- 3 points per service, max 10
  ...
  IF wealth_score IS NULL THEN
    wealth_score := 5; -- Default if zipcode not found
  END IF;
  
  -- Wealth index score (0-20 points)
  demographic_points := demographic_points + (wealth_score * 2.0); -- 2 points per wealth level
  
  -- Device type bonus (0-5 points)
  CASE profile_record.device_type
    WHEN 'desktop' THEN demographic_points := demographic_points + 5; -- More serious buyers
  ...
  IF array_length(profile_record.services_viewed, 1) >= 2 THEN
    intent_points := intent_points + 5;
  END IF;
